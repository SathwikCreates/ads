...
const STYLES: Record<string, Record<string, string>> = {
    trigger: {
        container:
            'rounded-full flex items-center bg-foreground justify-center cursor-pointer outline-none ring-0 hover:brightness-125 transition-all duration-100 z-50 shadow-xl',
        active: 'bg-foreground'
    },
    item: {
        container:
            'rounded-full flex items-center justify-center absolute bg-slate-900 text-white hover:bg-slate-800 cursor-pointer shadow-lg',
        label: 'text-xs font-bold text-slate-900 absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-white px-2 py-0.5 rounded-md shadow-sm whitespace-nowrap'
    }
};

const pointOnCircle = (i: number, n: number, r: number, cx = 0, cy = 0) => {
    // Start from top (multiply i by direction)
    // Shift angle to ensure distribution centers nicely
...
const MenuTrigger = ({
    ...
    return (
        <motion.div animate={shakeAnimation} className="z-50 border border-white/20 rounded-full">
            <motion.button
                animate={animate}
                style={{
                    height: CONSTANTS.itemSize,
                ...
                className={cn("bg-slate-900 text-white rounded-full flex items-center justify-center hover:scale-105 transition-transform", isOpen && STYLES.trigger.active)}
                onClick={() => {
                    if (isOpen) {
                        setIsOpen(false);
                        closeAnimationCallback();
...
export const CircleMenu = ({
    ...
    return (
        <div
            ...
            className={cn(
                "relative flex items-center justify-center pointer-events-none", // Container is pointer-events-none so it doesn't block clicks when closed
                isOpen && "pointer-events-auto" // Enable clicks when open
            )}
        >
            <div className="pointer-events-auto"> {/* Trigger is always clickable */}
                <MenuTrigger
                    setIsOpen={setIsOpen}
                    isOpen={isOpen}
